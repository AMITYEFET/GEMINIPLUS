let currentSettings = {
    linesEnabled: true, wrapEnabled: true, 
    resizerEnabled: true, clearEnabled: true, goToLineEnabled: true, 
    scrollbarEnabled: true, scrollbarColor: '#888888'
};
...
document.addEventListener('gemini-request-sync', () => {
    chrome.storage.sync.get(currentSettings, (data) => triggerMonacoSync(data));
});
...
function injectGeminiTools() {
    ...
    // למניעת לאגים: עדכון הכלים יתבצע רק אם הסרגל לא קיים או שיש שינוי בדף
    if (!wrapper) {
        ...
        // אם הוא כבר קיים ואין שינוי, אין טעם לרנדר מחדש ולאמץ את הזיכרון
    ...
    if (showGoToLine) {
        ...
        html += `
            <div class="gemini-goto-wrapper">
                <input type="number" id="geminiGotoInput" class="gemini-goto-input" placeholder="Line" min="1">
                <button class="gemini-resizer-btn gemini-goto-btn" title="Go to line" id="geminiGotoBtn">
    ...
    if (showGoToLine) {
        ...
        const gotoBtn = wrapper.querySelector('#geminiGotoBtn');
...
// מספור שורות מופרד וטהור כדי למנוע לאגים ופגיעה בהעתקה
function updateLineNumbers() {
    ...
    codeBlocks.forEach(pre => {
        ...
        // ספירת כמות השורות הטבעית מהטקסט הגולמי
        ...
        // מעדכן את ה-Gutter רק אם מספר השורות השתנה כדי לחסוך ביצועים
...
// אופטימיזציה קריטית: ה-Observer לא עוקב אחרי characterData כדי לא להיתקע בזמן הקלדה חיה
let debounceTimer;
const observer = new MutationObserver((mutations) => {
    ...
    if (shouldUpdate) {
        ...
        debounceTimer = setTimeout(() => {
            ...
            injectGeminiTools();
        }, 600); // השהייה חכמה שמונעת עומס בזמן בניית המסמך
    }
});
